...
export class PersistentScheduler {
  ...
  public async tick(now = new Date()): Promise<void> {
    ...
    try {
      ...
      await this.monitorService.recoverInterruptedRuns(new Date(now.getTime() - leaseMs));

      const events = this.db.prepare(`
        SELECT id, schedule, timezone, next_run_at
        FROM tracked_events
        WHERE status = 'active' AND next_run_at IS NOT NULL AND next_run_at <= ?
        ORDER BY next_run_at LIMIT 20
      `).all(nowIso) as DueEventRow[];
      ...
      for (const event of events) {
        ...
        try {
          ...
          `).run(nowIso, event.id, event.next_run_at);
          continue;
        }
        const claimed = this.db.prepare(`
          UPDATE tracked_events SET next_run_at = ?, updated_at = ?, version = version + 1
          WHERE id = ? AND next_run_at = ? AND status = 'active'
        `).run(nextRunAt, nowIso, event.id, event.next_run_at);
        if (claimed.changes === 1) {
          await this.monitorService.runEvent(event.id, event.next_run_at);
        }
      }

      const expectedAssets = this.db.prepare(`
        SELECT id, next_check_at FROM expected_assets
        WHERE stage NOT IN ('claimed', 'missed', 'expired', 'rejected')
          AND next_check_at <= ?
        ORDER BY next_check_at LIMIT 20
      `).all(nowIso) as DueExpectedRow[];
      for (const expected of expectedAssets) {
        ...
        `).run(claimedNextCheck, nowIso, expected.id, expected.next_check_at);
        if (claimed.changes === 1) {
          await this.monitorService.runExpectedCheck(expected.id, expected.next_check_at);
        }
      }

      await this.emailOutbox.processDue();
    } finally {
      this.ticking = false;
    }
  }
