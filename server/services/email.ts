  ...
  public enqueue(input: {
    ...
    this.db.prepare(`
      INSERT OR IGNORE INTO email_outbox (
        id, run_id, recipient, subject, text_body, html_body, status,
    ...
    this.db.prepare("UPDATE monitor_runs SET email_status = ? WHERE id = ?").run(status, input.runId);
    return status;
  }
  ...
  public async processDue(limit = 10): Promise<void> {
    ...
    if (!configured) {
      ...
      this.db.prepare(`
        ...
        WHERE id IN (SELECT run_id FROM email_outbox WHERE status = 'skipped')
      `).run();
      return;
    }
      ...
      try {
        ...
        this.db.prepare(`
          ...
          SET status = 'sent', attempts = attempts + 1, provider_message_id = ?,
              last_error = '', sent_at = ?
          WHERE id = ?
        `).run(result.messageId ?? "", now, row.id);
        if (row.run_id) {
          this.db.prepare("UPDATE monitor_runs SET email_status = 'sent' WHERE id = ?").run(row.run_id);
        }
      } catch (error) {
        ...
        this.db.prepare(`
          ...
          WHERE id = ?
        `).run(terminal ? "failed" : "pending", attempts, nextAttempt, message.slice(0, 2_000), row.id);
        if (terminal && row.run_id) {
          this.db.prepare("UPDATE monitor_runs SET email_status = 'failed' WHERE id = ?").run(row.run_id);
        }
      }
    }
  }
